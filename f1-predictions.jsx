...
async function getMeetings() {
  const data = await fetchJSON(`${F1_API}/meetings?year=2025`);
  return data || [];
}
...
export default function F1Predictions() {
  ...
  async function bootstrap() {
    ...
    if (qSess) {
      const qRes = await getPositions(qSess.session_key);
      setQualiResults(enrichResults(qRes, drivers));
    }
    if (rSess) {
      const rDrivers = drivers.length ? drivers : await getDrivers(rSess.session_key);
      setRaceResults(enrichResults(await getPositions(rSess.session_key), rDrivers));
    }
    if (sprintWeekend && sSess) {
      const sDrivers = drivers.length ? drivers : await getDrivers(sSess.session_key);
      setSprintResults(enrichResults(await getPositions(sSess.session_key), sDrivers));
    }
    setLoading(false);
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0f", fontFamily: "'Barlow Condensed','Arial Narrow',sans-serif", color: "#fff", overflowX: "hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .error-bar{background:rgba(255,50,50,.1);border:1px solid rgba(255,50,50,.3);color:#ff8080;font-size:12px;padding:10px 16px;border-radius:6px;margin-bottom:16px;letter-spacing:.05em}
      `}</style>
                  ...
                  {PLAYERS.filter(p => eventPreds[p]).map(player => {
                    ...
                    return (
                      <div key={player} style={{ background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.07)", borderRadius:8, padding:"12px 16px", display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                        <div style={{ fontWeight:800, fontSize:15, letterSpacing:"0.05em", flex:1 }}>{player}</div>
                        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          <div className="slide-in" style={{ padding:"24px 0 60px" }}>
            ...
            {currentSession && (
              <div style={{ background:"rgba(225,6,0,.08)", border:"1px solid rgba(225,6,0,.2)", borderRadius:8, padding:"12px 16px", marginBottom:20, display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
                <div>
                  <div style={{ fontSize:18, fontWeight:800, letterSpacing:"0.08em" }}>{currentSession.meeting_name}</div>
            ...
            {loading ? (
              ...
              <div style={{ display:"flex", gap:12, padding:"4px 12px", marginBottom:4 }}>
                {["POS","","DRV","DRIVER","TEAM"].map((h,i) => (
                  <div key={i} style={{ fontSize:9, color:"#555", letterSpacing:"0.15em", width:i===0?32:i===1?3:i===2?48:i===3?"flex":120, flex:i===3?1:undefined }}>{h}</div>
                ))}
              </div>
              {currentResults.slice(0,20).map((r, idx) => {
                ...
                return (
                  <div key={r.driver_number} className={`live-row${isTop3?" top3":""}`}>
                    ...
                    <div style={{ width:48, fontWeight:800, fontSize:14, letterSpacing:"0.08em", color }}>{r.name_acronym||`#${r.driver_number}`}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:600, fontSize:14 }}>{r.full_name||"—"}</div>
                      {predictors.length>0 && <div style={{ fontSize:10, color:"#e10600", fontWeight:700, letterSpacing:"0.1em", marginTop:2 }}>⬡ {predictors.join(", ")}</div>}
                    </div>
                    <div style={{ fontSize:11, color:"#555", maxWidth:120, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{r.team_name||""}</div>
                    {isTop3 && <div style={{ width:8, height:8, borderRadius:"50%", background:color, flexShrink:0 }}/>}
                  </div>
                );
              })}
              ...
              {Object.keys(eventPreds).length>0 && (
                <div style={{ marginTop:32 }}>
                  ...
                  <div style={{ fontSize:10, color:"#444", marginTop:8, letterSpacing:"0.1em" }}>Green = exact position · Orange = in top 3</div>
                </div>
              )}
            </>)}
          </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                {leaderboard.map((entry, idx) => (
                  <div key={entry.name} className={`lb-row${idx===0?" first-place":""}`}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:800, fontSize:18, letterSpacing:"0.05em" }}>{entry.name}</div>
    ...
    <div ref={ref} style={{ position:"relative" }}>
      <div className={`pick-slot${value?" filled":""}`} onClick={() => setOpen(o=>!o)}>
        {selected ? (<>
          ...
          <div style={{ fontWeight:800, fontSize:13, letterSpacing:"0.08em", color:teamColor(selected.team_name||"") }}>{selected.name_acronym}</div>
